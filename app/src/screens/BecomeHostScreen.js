...
export default function BecomeHostScreen({ navigation }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setIp(address);
        const { port: p } = await networkService.becomeHost(token);
        setPort(p);
      } catch (err) {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>You're the Host</Text>
      <Text style={styles.subtitle}>Give these two things to every other phone:</Text>
      ...
      <Text style={styles.hint}>
        This device keeps working exactly as before — hosting just lets other
        phones connect to it too. Stay on the same WiFi or hotspot the other
        phones will join. You can see this again anytime in Sync Settings.
      </Text>
