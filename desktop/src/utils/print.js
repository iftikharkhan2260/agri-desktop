...
export async function generatePdfAndShare(html, suggestedName) {
  const result = await window.desktopAPI.exportPdf(html, suggestedName || 'export.pdf');
  return result?.filePath;
}

// For "Print Receipt" buttons — opens the OS print dialog.
export async function printHtml(html) {
  await window.desktopAPI.printHtml(html);
}
