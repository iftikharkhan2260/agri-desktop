...
export default function GuarantorFormScreen({ route, navigation }) {
  const { t, fontFamily, isRTL } = useLang();
  const existing = route.params?.guarantor;
  ...
  async function handleSubmit() {
    ...
    setSaving(true);
    try {
      const payload = { name, company_name: companyName, contact, address };
      if (isEdit) {
        await api.put(`/api/guarantors/${existing.id}`, payload);
      } else {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ padding: spacing.lg }}>
      ...
      <Text style={[styles.label, { fontFamily: fontFamily('medium') }, align]}>{t('companyName')}</Text>
      <TextInput
